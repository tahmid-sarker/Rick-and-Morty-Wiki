...
const Episodes = () => {
    const { episodes } = use(DataContext)
    // console.log(episodes);
    const swiperRef = useRef(null);
    return (
        <div className="w-10/12 mx-auto">
            <h1 className='text-lg md:text-2xl text-white mb-5 md:mb-9'>Episodes</h1>
                ...
                <Swiper modules={[Navigation]} breakpoints={{
                    ...
                    {episodes.map((episode, index) => (
                        <SwiperSlide key={index} className='border p-2 mt:p-4 border-primary w-16 h-16 rounded-br-2xl md:rounded-br-4xl'>
                            <p className='text-[10px] md:text-sm text-white mt-2 truncate'>{episode.episode}</p>
                            <p className='text-[10px] md:text-sm text-white mt-2 truncate'>{episode.name}</p>
                        </SwiperSlide>
                    ))}
                </Swiper>
                {/*Right Arrow Button*/}
